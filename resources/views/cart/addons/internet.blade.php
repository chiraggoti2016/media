<!-- Modem start -->
@php $_modemdata = isset($cart['addon']['internet']['modem']) ? $cart['addon']['internet']['modem'] : []; @endphp
 ...
 <div class="container alternate">
 	...
 	@if(isset($addons['modem']) && $modems = $addons['modem'])
	    ...
	    <div class="row">
	    	...
	    	<div class="col-sm-3 text-center"><a href="#" class="select-btn {{ (in_array($_modem->id,$_modemkeys)?'select':'') }}" data-modem-id="{{$_modem->id}}" data-url="{{route('cart.add.addon','internet')}}" id="modem-confirm-btn">Confirm</a></div>
	    </div>
	    @endforeach
 	@endif
...
<!-- Other Addos start -->
@php $_otherdata = isset($cart['addon']['internet']['other']) ? $cart['addon']['internet']['other'] : []; @endphp
	    ...
	    <div class="row">
	    	<div class="col-sm-3"><img src="{{ asset('storage/' . $_other->image) }}"/></div>
	      	...
	      	@if(in_array($_other->id,$_otherkeys))
	      		<div class="col-sm-3 text-center"><a href="#" class="select-btn" data-other-id="{{$_other->id}}" data-url="{{route('cart.remove.addon','internet')}}" id="other-confirm-btn">Cancel</a></div>
	      	@else 
	      		<div class="col-sm-3 text-center"><a href="#" class="select-btn" data-other-id="{{$_other->id}}" data-url="{{route('cart.add.addon','internet')}}" id="other-confirm-btn">Confirm</a></div>
	      	@endif
